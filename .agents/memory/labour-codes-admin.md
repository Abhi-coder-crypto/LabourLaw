...
- Vite dev server config sets `server.strictPort: true`. Why: without it, Vite silently falls back to another port if 5000 is busy, which breaks the Replit workflow (waits on 5000) and webview. Keep this set whenever touching `vite.config.ts`.
